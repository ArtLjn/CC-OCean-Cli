...
export async function loadMemoryPrompt(): Promise<string | null> {
  ...
  return [
    ...
    '',
    '## 存储规则',
    '- 用户身份/画像 → category="identity"（全局，始终注入）',
    '- 编码习惯 → category="coding_style"（全局，按技术栈匹配注入）',
    '- 工具偏好 → category="tool_pref"（全局，按需检索）',
    '- 工作流偏好 → category="workflow"（全局，始终注入）',
    '- 项目知识 → category="project"（项目级，跟随项目）',
    '- 通用事实 → category="general"（全局，按需检索）',
    '',
    '## 操作',
